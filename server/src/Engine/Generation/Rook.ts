...
// Generates which tiles a Rook can attack from the given square, taking into account blocking bits
export function generateRookFlyAttacks(square: number, block: bigint) {
  ...
  for (let rank = targetRank + 1; rank <= 7; rank++) {
    ...
    if ((BigInt(rank) * 8n + BigInt(targetFile)) && block) break;
  }
  ...
  for (let file = targetFile + 1; file <= 7; file++) {
    ...
    if ((BigInt(file) * 8n + BigInt(targetFile)) && block) break;
  }
...
// Generates which tiles a Rook can attack from the given square, used for Magic BitBoard
